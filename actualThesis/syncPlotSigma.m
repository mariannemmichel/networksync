...
function syncPlotSigma(T,Y,N,C,thresh,threshTau,sigmaW,varargin)
    ...
    if doDendro
        ...
        for s=1:numS
            ...
            title(['Dendrogram for sigma = ' num2str(sigmaW(s))])
            pause
        end
    end
    ...
    if doState
        ...
        for s=1:numS
            ...
            title(['States of synchronization for sigma = ' num2str(sigmaW(s))])
            pause
        end
    end
    ...
    if doTimesquares
        ...
        for s=1:numS
            ...
            title(['Sync time of nodes for sigma = ' num2str(sigmaW(s))])
            pause
        end
    end
    ...
    if doTrajec
        ...
        for s=1:numS
            ...
            title(['Trajectories of nodes for sigma = ' num2str(sigmaW(s))])
            pause
        end
    end
...
function tauPlot(sigmaW,tauC,L)
    plot(sigmaW,tauC,'.')
    xlabel('\sigma')
    ylabel('\tau')
    if tauC < 20
